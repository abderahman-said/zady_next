...
{getBannersData?.map((e) => (
  <div key={e.id}>
     <div className={styles.coffetwo} >
       <LazyLoadImage loading="lazy"src={`/api/images?id=${e.verticalImage}`} alt={e.name} width={67600} height={67600} className='img-verticalImage' data-aos="zoom-in" />
      </div>
           {/* ==============abouthree============= */}
           <Row>
            <Col md={3} >
              <LazyLoadImage loading="lazy"src={`/api/images?id=${e.horizontalImage}`} alt={e.name} width={67600} height={67600} className='img-horizontalImage' data-aos="zoom-in" />
            </Col>
            <Col md={9}>
            <div className='taps-cats'>
     {e.cats.map(cat => (
          <div className='taps-cats-Banners' key={cat.id} onClick={() => setSelectedCatId(cat.id)}data-aos="zoom-in" >
              {cat.name} 
          </div>
     ))}
        </div>
